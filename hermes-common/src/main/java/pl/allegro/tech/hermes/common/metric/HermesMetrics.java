...
public class HermesMetrics {
    ...
    public Timer timer(String metric) {
        return metricRegistry.timer(MetricRegistry.name(metricName(metric)));
    }

    public Timer timer(String metric, TopicName topicName) {
        return metricRegistry.timer(metricRegistryName(metricName(metric), topicName));
    }

    public Timer timer(String metric, TopicName topicName, String name) {
        return metricRegistry.timer(metricRegistryName(metricName(metric), topicName, name));
    }

    public Meter meter(String metric) {
        return metricRegistry.meter(MetricRegistry.name(metricName(metric)));
    }

    public Meter meter(String metric, TopicName topicName, String name) {
        return metricRegistry.meter(metricRegistryName(metricName(metric), topicName, name));
    }

    public Meter meter(String metric, TopicName topicName) {
        return metricRegistry.meter(metricRegistryName(metricName(metric), topicName));
    }
    ...
    public Counter counter(String metric, TopicName topicName) {
        return metricRegistry.counter(metricRegistryName(metricName(metric), topicName));
    }

    public Counter counter(String metric, TopicName topicName, String name) {
        return metricRegistry.counter(metricRegistryName(metricName(metric), topicName, name));
    }
    ...
    public Counter counterForOffsetLag(Subscription subscription, int partition) {
        return metricRegistry.counter(pathCompiler.compile(Counters.CONSUMER_OFFSET_LAG, pathContext()
                .withGroup(subscription.getTopicName().getGroupName())
                .withTopic(subscription.getTopicName().getName())
    ...
    public Counter counterForOffsetCommitIdlePeriod(Subscription subscription, int partition) {
        return metricRegistry.counter(offsetCommitIdleMetricName(subscription, partition));
    }

    public void removeCounterForOffsetCommitIdlePeriod(Subscription subscription, int partition) {
        metricRegistry.remove(offsetCommitIdleMetricName(subscription, partition));
    }

    private String offsetCommitIdleMetricName(Subscription subscription, int partition) {
        return metricRegistryName(
            metricName(Counters.CONSUMER_OFFSET_COMMIT_IDLE),
    ...
    public void registerConsumersThreadGauge(Gauge<Integer> gauge) {
        metricRegistry.register(pathCompiler.compile(Gauges.CONSUMER_THREADS), gauge);
    }

    public <T> void registerOutputRateGauge(TopicName topicName, String name, Gauge<T> gauge) {
        String normalizedMetricName = metricRegistryName(pathCompiler.compile(Gauges.CONSUMER_OUTPUT_RATE), topicName, name);
    ...
    public double getConsumerMeterOneMinuteRate(Subscription subscription) {
        return meter(Meters.CONSUMER_METER, subscription.getTopicName(), subscription.getName()).getOneMinuteRate();
    }
    ...
    private double getDoubleValue(String gauge) {
        return (double) metricRegistry.getGauges().get(metricName(gauge)).getValue();
    }
    ...
    private String metricRegistryName(String metricDisplayName, TopicName topicName, String name, String subname) {
        return MetricRegistry.name(
            metricDisplayName, escapeDots(topicName.getGroupName()), escapeDots(topicName.getName()), escapeDots(name), escapeDots(subname)
        );
    }
    ...
    public void registerGauge(String name, Gauge<?> gauge) {
        metricRegistry.register(metricName(name), gauge);
    }

    private String metricName(String staticPostfix) {
        return pathCompiler.compile(staticPostfix);
    }
